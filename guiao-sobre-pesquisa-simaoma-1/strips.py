...
class STRIPS(SearchDomain):
    ...
    def result(self, state, action):
        new_state = state.copy()  # Create a copy of the state to avoid modifying the original
        # Apply the action's effects to the new state
        for effect in action.pos:
            if effect not in new_state:  # Avoid duplicates
                new_state.append(effect)
        for effect in action.neg:
            if effect in new_state:  # Remove negatives if they exist
    ...
    def satisfies(self, state, goal):
        return all(condition in state for condition in goal)
